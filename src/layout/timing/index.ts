import type { TimingAst, TimingEvent, TimingTrack } from '../../ast/timing.js';
...
export function layoutTiming(ast: TimingAst): Scene {
  ...
  const laneWidth = Math.max(MIN_SEGMENT_PX * segments, 240);
  const laneOriginX = PAGE_PAD + labelW + LABEL_PAD_X;
  const totalW = laneOriginX + laneWidth + PAGE_PAD;
  const totalH = PAGE_PAD + titleH + totalLaneH + AXIS_H + PAGE_PAD;

  const xOfTime = (t: number): number => laneOriginX + ((t - minT) / span) * laneWidth;
...
function drawStateLane(
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (e.state) {
      const fitted = fitLabel(e.state, w - 6, STATE_FONT);
      if (fitted !== null) {
        shapes.push({
          type: 'text',
          x: x1 + w / 2,
...
function drawBinary(
  ...
  for (let i = 0; i < events.length; i++) {
    const e = events[i]!;
    const level = binaryLevel(e.state);
    const nextT = i + 1 < events.length ? events[i + 1]!.time : maxT;
    const x1 = xOf(e.time);
    const x2 = xOf(nextT);
